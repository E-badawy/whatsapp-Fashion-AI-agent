...
DB_PATH = Path(os.getenv("DATABASE_PATH", str(DATA_DIR / "fashion_agent.db")))
...
def init_db():
    with get_connection() as conn:
        ...
        existing_skus = {
            row["sku"]
            for row in conn.execute("SELECT sku FROM products WHERE sku IS NOT NULL").fetchall()
        }
        required_skus = {
            "DRS-001",
            "KFT-014",
        ...
        if not required_skus.issubset(existing_skus):
            seed_products(conn)
        else:
            for sample in [
                {
                ...
                create_product(sample, conn=conn)
...
def seed_products(conn):
    ...
    story_count = conn.execute("SELECT COUNT(*) AS count FROM story_items").fetchone()["count"]
    if story_count == 0:
        conn.execute(
            """
...
def list_products():
    with get_connection() as conn:
        rows = conn.execute(
            "SELECT * FROM products ORDER BY is_featured DESC, stock DESC, created_at DESC"
        ).fetchall()
...
def delete_story_item(story_id):
    with get_connection() as conn:
        conn.execute("DELETE FROM story_items WHERE id = ?", (story_id,))
        conn.commit()
...
def update_order_status(order_id, status, payment_status=None, notes=None):
    with get_connection() as conn:
        updates = ["status = ?"]
        values = [status]
