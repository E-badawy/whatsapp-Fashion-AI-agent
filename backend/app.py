...
from services import meta_catalog
...
def public_url_for(path_or_url):
    if not path_or_url:
        return ""
    ...
    if path_or_url.startswith("/"):
        ...
        return f"{request.host_url.rstrip('/')}{path_or_url}"
    return path_or_url
    ...
    for incoming in messages:
        ...
        if wants_image(message):
            ...
            if products:
                ...
                if image_urls:
                    image_url = public_url_for(image_urls[0])
                    whatsapp.send_image(
                        phone,
                        image_url,
...
def delete_story(story_id):
    existing = [story for story in db.list_story_items() if story.get("id") == story_id]
    if not existing:
        return jsonify({"error": "story not found"}), 404
    db.delete_story_item(story_id)
