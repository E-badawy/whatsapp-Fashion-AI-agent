...
let editingSku = null;
let uploadedImages = [];
let theme = localStorage.getItem("fashion-theme") || "light";
...
function setTheme(nextTheme) {
  theme = nextTheme;
  localStorage.setItem("fashion-theme", theme);
  applyTheme();
}

function addBubble(role, text, imageUrl = null, caption = "") {
  ...
  if (imageUrl) {
    ...
    image.src = imageUrl;
    image.alt = caption || text || "Product image";
...
async function loadProducts() {
    ...
    const products = await api(`/api/products${search ? `?q=${encodeURIComponent(search)}` : ""}`);
...
async function loadStories() {
      ...
      (story) => `
        <article class="item">
          ...
          <div style="margin-top:10px;display:flex;justify-content:flex-end;">
            <button type="button" class="deleteStory" data-story-id="${story.id}">Delete</button>
          </div>
        </article>
      `
    )
  ...
  document.querySelectorAll(".deleteStory").forEach((button) => {
    button.onclick = () => deleteStory(button.dataset.storyId);
  });
}

async function deleteStory(storyId) {
  if (!confirm("Delete this story item?")) return;
  await api(`/api/stories/${storyId}`, { method: "DELETE" });
  await loadStories();
}

async function loadOrders() {
  const search = (qs("#orderSearch")?.value || "").trim();
  const orders = await api(`/api/orders${search ? `?q=${encodeURIComponent(search)}` : ""}`);
        ...
        (order) => `
          <article class="item">
            ...
            <div class="order-actions">
              <button type="button" data-order-id="${order.id}" data-status="pending">Pending</button>
              <button type="button" data-order-id="${order.id}" data-status="confirmed">Confirmed</button>
              <button type="button" data-order-id="${order.id}" data-status="shipped">Shipped</button>
              <button type="button" data-order-id="${order.id}" data-status="completed">Completed</button>
            </div>
          </article>
        `
      )
...
async function boot() {
  ...
  qs("#themeToggle").addEventListener("click", () => setTheme(theme === "dark" ? "light" : "dark"));
  qs("#refreshProducts").addEventListener("click", loadProducts);
  qs("#refreshOrders").addEventListener("click", loadOrders);
  qs("#clearOrders").addEventListener("click", async () => {
    if (!confirm("Clear all orders?")) return;
    await api("/api/orders", { method: "DELETE" });
    await loadOrders();
  });
  qs("#refreshConversations").addEventListener("click", loadConversations);
  qs("#productSearch").addEventListener("input", () => loadProducts());
  qs("#orderSearch").addEventListener("input", () => loadOrders());
...
loadProducts();
  ...
  qs("#chatForm").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      addBubble("assistant", result.reply, result.image_url, result.caption);
      await loadConversations();
    } catch (error) {
  ...
  document.addEventListener("click", async (event) => {
    const orderButton = event.target.closest("button[data-order-id]");
    if (orderButton) {
      await updateOrderStatus(orderButton.dataset.orderId, orderButton.dataset.status);
      return;
    }
  });
